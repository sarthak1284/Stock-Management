...
def sale_laptops(listed_items):
    ...
    while laptop_selling:
        ...
        if laptop_is_added:
            ...
            for i in range(len(list_data)):
                ...
                if str(laptopID) == list_data[i][0] and quantity <= int(list_data[i][4]):
                    dictionary_data[laptopID] = quantity
                    break
                elif quantity >= int(list_data[i][4]):
        ...
        while True:
            print()
            # Asking the user if they'd wish to sell any more Laptops
    ...
    # Asking the user if they want their laptop to be shipped
    while True:
        # Exception Handling till the user enters a valid string value
        try:
            shipping = input("Do you want your laptop to be shipped? (Y/N) ").upper()
            if shipping == "Y":
                print("$500 will be added to your total amount")
                shipping_cost = 500
                ...
                shipping_cost = 0 
                print("Thus, the calculated amount is: " + "$" + str(amount))
                break
        except:
    ...
    file_write.write("\n")
    file_write.write("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t     Grand_Total: " + "$" + str(amount))
    file_write.write("\n")
    file_write.write("\n")
    file_write.write(
        "\t\t\t --------------------------------------------------------------------------- THANKYOU ---------------------------------------------------------------------------")
